...
export function* fetchAttachments({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/projects/actions/${id}/attachments`);
...
export function* fetchComments({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/projects/actions/${id}/comments`);
...
export function* fetchImages({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/projects/actions/${id}/images`);
...
export function* fetchRating({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/projects/actions/${id}/rating`);
...
export function* setRate({ payload }: any) {
  try {
    const response: any = yield* call(service.client.post, `/projects/actions/rating`, payload);
...
export function* submitNew({ payload }: any) {
  try {
    const response: any = yield* call(service.client.post, '/projects/actions/comments', payload);
    yield put(actions.loadSubmitNewSuccess(response.data));
    const responseList: any = yield* call(service.client.get, `/projects/actions/${payload.idProject}/comments`);
...
export function* deleteComment({ payload }: any) {
  ...
  try {
    const response: any = yield* call(service.client.delete, `/projects/actions/${idProject}/comments/${id}`);
    yield put(actions.loadDeleteCommentSuccess(response.data));
    const responseList: any = yield* call(service.client.get, `/projects/actions/${idProject}/comments`);
    yield put(actions.loadFetchCommentsSuccess(responseList.data));
  } catch (err) {
    const parser = parseError(err.error.response);
