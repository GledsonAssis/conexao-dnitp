...
export function* highlights() {
  try {
    const response: any = yield* call(service.client.get, '/courses');
...
export function* fetchAttachments({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/courses/${id}/attachments`);
...
export function* fetchComments({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/courses/${id}/comments`);
...
export function* fetchImages({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/courses/${id}/images`);
...
export function* fetchRating({ payload }: any) {
  const { id } = payload;
  try {
    const response: any = yield* call(service.client.get, `/courses/${id}/rating`);
...
export function* setRate({ payload }: any) {
  try {
    const response: any = yield* call(service.client.post, `/courses/rating`, payload);
...
export function* submitNew({ payload }: any) {
  try {
    const response: any = yield* call(service.client.post, '/courses/comments', payload);
    yield put(actions.loadSubmitNewSuccess(response.data));
    const responseList: any = yield* call(service.client.get, `/courses/${payload.idCourse}/comments`);
...
export function* deleteComment({ payload }: any) {
  const { idCourse, id } = payload;
  try {
    const response: any = yield* call(service.client.delete, `/courses/${idCourse}/comments/${id}`);
    yield put(actions.loadDeleteCommentSuccess(response.data));
    const responseList: any = yield* call(service.client.get, `/courses/${idCourse}/comments`);
    yield put(actions.loadFetchCommentsSuccess(responseList.data));
  } catch (err) {
    const parser = parseError(err.error.response);
