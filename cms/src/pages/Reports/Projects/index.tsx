...
import { getCategoryProjects, getTypeProject } from '@/infra/constants/reportsTypes';
...
export const ReportProjectsPage: React.FC<Props> = ({ propsModel, t }) => {
  ...
  useEffect(() => {
    dispatch(actionsReports.loadReportsProjectsListRequest({
      type: watch('type'),
  ...
  function renderList() {
    if (reportsList?.projects?.length) {
      const data = sortItens(reportsList?.projects);
      const listFiltered = changePage(currentPage, numberItensPer, data)
      return listFiltered.map((item, index) => (
        <tr key={`reports_projects_${item?.['Título']}_${index}`}>
          <td className={'px-0'} />
          <td className="pl-4 text-truncate" data-th={t('pages:Reports.Projects.Table.Category')}>
            {item?.Categoria || '-'}
          </td>
          <td className="w-100 text-truncate" data-th={t('pages:Reports.Projects.Table.TableTitle')}>
            {item?.['Situação'] || '-'}
          </td>
          <td data-th={t('pages:Reports.Projects.Table.Situation')}>
            {item?.['Superintendência Regional'] || '-'}
          </td>
          <td data-th={t('pages:Reports.Projects.Table.Superintendence')}>
            {item?.['Título'] || '-'}
          </td>
          <td data-th={t('pages:Reports.Projects.Table.LocalUnit')}>
            {item?.['Unidade Local'] || '-'}
          </td>
        </tr>
      ));
      ...
      return <tr className="empty-data">
        <td colSpan={9} className="py-5">
          ...
          {t('pages:Reports.Projects.Table.EmptyList')}
        </td>
      </tr>
    }
  }
  ...
  function renderCategory() {
    return getCategoryProjects()?.map((item: { value: string, title: string }) => (
      <div key={`category-select-${item.value}`} className="br-item w-100" tabIndex={-1}>
        <div className="br-radio">
          <input
  ...
  function renderSituation() {
    return getTypeProject()?.map((item: { value: boolean, title: string }) => (
    ...
    <div className="main-content mt-3 px-md-3" id="main-content">
      <div className="row">
        <div className='col-12 d-flex justify-content-between align-items-center'>
          <p className="h1">{t('pages:Reports.Projects.Table.Title')}</p>
        </div>
            ...
            <div className="w-100 my-4">
                ...
                <div className='row'>
                  <div className='col-6'>
                    <div className={`br-input medium`}>
                      <label htmlFor="Superintendence">{t('pages:Reports.Projects.Labels.Superintendence')}:</label>
                      <Select
                        id="Superintendence"
                        style={{ maxHeight: 320 }}
                  ...
                  <div className='col-6'>
                    <div className={`br-input medium`}>
                      <label htmlFor="LocalUnit">{t('pages:Reports.Projects.Labels.LocalUnit')}:</label>
                      <Select
                        id="LocalUnit"
                        style={{ maxHeight: 320 }}
                        CustomclassName={`mw-100`}
                  ...
                  <div className='col-6'>
                    <div className={`br-input medium`}>
                      <label htmlFor="situation">{t('pages:Reports.Projects.Labels.Situation')}:</label>
                      <Select
                        id="situation"
                        style={{ maxHeight: 320 }}
                        CustomclassName={`mw-100`}
                  ...
                  <div className='col-6'>
                    <div className={`br-input medium`}>
                      <label htmlFor="category">{t('pages:Reports.Projects.Labels.Category')}:</label>
                      <Select
                        id="category"
                        style={{ maxHeight: 320 }}
                        CustomclassName={`mw-100`}
                  ...
                  <div className='col-4'>
                    <div className={`br-input medium`}>
                      <label htmlFor="period">{t('pages:Reports.Projects.Labels.TimeCourse')}:</label>
                      <Select
                        id="period"
                        style={{ maxHeight: 320 }}
                        CustomclassName={`mw-100`}
                  ...
                  <div className={`br-input col-12 col-md-4 ${errors?.From?.type ? 'danger' : ''}`}>
                    <label htmlFor="period">{t('pages:Reports.Projects.Labels.From')}:</label>
                    <Datapicker
                      id="From"
                      minDate="all"
                      maxDate="today"
                  ...
                  <div className={`br-input col-12 col-md-4 ${errors?.To?.type ? 'danger' : ''}`}>
                    <label htmlFor="period">{t('pages:Reports.Projects.Labels.To')}:</label>
                    <Datapicker
                      id="To"
                      minDate="all"
                      maxDate="today"
              ...
              <div className="d-flex">
                <button className="br-button ml-sm-auto" type="button">
                  {t('pages:Reports.Projects.Labels.Clear')}
                </button>
                <button className="br-button primary ml-sm-2" type="submit">
                  {t('pages:Reports.Projects.Labels.Search')}
                </button>
              </div>
            </div>
          </form>
          <Table
            Title={t('pages:Reports.Projects.Table.Title')}
            Options={{
              id: 'projects',
              paginate: true,
              count: reportsList?.projects?.length,
            }}
            translations={t}
          ...
          >
            ...
            <THead nameKey='t-head'>
              <tr>
                ...
                <th scope="col" className={'text-nowrap pl-4'}><span>{t('pages:Reports.Projects.Table.Category')}</span></th>
                <th scope="col" className={'text-nowrap'}><span>{t('pages:Reports.Projects.Table.TableTitle')}</span></th>
                <th scope="col" className={'text-nowrap'}><span>{t('pages:Reports.Projects.Table.Situation')}</span></th>
                <th scope="col" className={'text-nowrap'}><span>{t('pages:Reports.Projects.Table.Superintendence')}</span></th>
                <th scope="col" className={'text-nowrap'}><span>{t('pages:Reports.Projects.Table.LocalUnit')}</span></th>
              </tr>
            </THead>
