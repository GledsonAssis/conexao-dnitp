...
export const EditInitiativePage: React.FC<Props> = ({ propsModel, t }) => {
  ...
  function onSubmit(dataSubmit: any) {
    dispatch(actionsInitiatives.loadSubmitRequest({ ...dataSubmit, idInitiative: +id, idInitiativeEvaluation: data.evaluation.id, typeList: 'publish' }));
  }
  ...
  return (
    <>
      {`${data?.id}` === id ?
        <Form
          titlePage={t('pages:initiatives.details.pageTitle.publish')}
