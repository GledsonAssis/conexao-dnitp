...
export const ProjectsPage: React.FC<Props> = ({ propsModel, t }) => {
  ...
  useEffect(() => {
    dispatch(actionsActions.loadListRequest({
      page: currentPage,
      limit: numberItensPer,
      keyword: keywordSearch
    }));
    // eslint-disable-next-line
  }, [currentPage, numberItensPer, actionsList.dataId, keywordSearch]);
  ...
  function RenderItemStatus(params) {
    ...
    if (item.isPublished) {
      ...
      statusLabel = t('pages:actions.labels.table.published');
    } else if (item.excludedDate) {
      className = 'inactivated-status-table danger';
      statusLabel = t('pages:actions.labels.table.inactivated');
    } else {
      className = 'registered-status-table warning';
      statusLabel = t('pages:actions.labels.table.registered');
    }
  ...
  function renderList() {
    if (actionsList && actionsList.data?.count) {
      return actionsList.data.rows.map((item) => (
        <tr key={`project_${item.id}`}>
          <td className={'px-0'} />
          <td className="w-100 pl-4" data-th={t('pages:actions.labels.table.projectName')}>
            {item.title}
          </td>
          <td data-th={t('pages:actions.labels.table.actions')}>
            <Link
              href={{
            ...
            <button
              ...
              onClick={() => dispatch(actionsActions.loadPublishRequest({ id: item.id, isPublished: !item.isPublished }))}>
              {item.isPublished ?
          ...
          <td data-th={t('pages:actions.labels.table.situation')}>
    ...
    <div className="main-content mt-3 px-md-3" id="main-content">
      <div className="row">
        <div className='col-12 d-flex justify-content-between align-items-center'>
          <p className="h1">{t('pages:actions.labels.pageTitle')}</p>
          <Link href={`${router.asPath}/novo`}>
            <button className="br-button secondary" type="button">
              ...
              {t('pages:actions.labels.btnCreate')}
            </button>
          </Link>
        </div>
        <div className='col-12'>
          ...
          <Table
            Title={t('pages:actions.labels.listTitle')}
            Options={{
              id: 'actions',
              paginate: true,
              count: actionsList.data?.count,
          ...
          >
            <ActionTrigger nameKey='top-bar' className={'mr-1'}>
              <button
                ...
                type="button"
                onClick={() => dispatch(actionsActions.loadGetListCSVsRequest({ keyword: keywordSearch }))}>
            ...
            <THead nameKey='t-head'>
              <tr>
                ...
                <th scope="col"><span>{t('pages:actions.labels.table.title')}</span></th>
                <th scope="col"><span>{t('pages:actions.labels.table.actions')}</span></th>
                <th scope="col"><span>{t('pages:actions.labels.table.situation')}</span></th>
              </tr>
            </THead>
