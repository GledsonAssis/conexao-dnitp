...
class BRTable {
  ...
  constructor(name: string, component: any, sequence: any) {
    this.name = name;
  ...
  _makeResponsiveTable() {
    ...
    if (!this.component.querySelector(`.${responsiveClass}`) && this._table) {
  ...
  _hideThead() {
    this._table.style.marginTop = `-${this._table.querySelector('thead').offsetHeight}px`;
  }
  ...
  _dropdownClose(element?) {
    const elem = element ? [element] : this.component.querySelectorAll('[data-toggle="dropdown"]');
    for (const elemDrop of elem) {
      elemDrop.setAttribute('aria-expanded', 'false');
      elemDrop.nextElementSibling.classList.remove('show');
    }
  }

  _searchBehavior() {
    if (this.component && this.component.dataset.search) {
      ...
      if (target) {
        ...
        if (trigger) {
          ...
          trigger.addEventListener('click', () => this._searchOpen(trigger, target));
        }

        // Fecha busca
        if (dismiss) {
          dismiss.addEventListener('click', () => this._searchClose(trigger, target));
        }
  ...
  _checkAllClick(elemName) {
    ...
    this._checkAllTable(selectedBar, tableCheckboxes, headerCheckbox, elemName);
  }

  _setRow(checkbox, check) {
    const headerCheckbox = this.component.querySelector('thead [type="checkbox"]');

    if (headerCheckbox) {
      const trList = this.component.querySelectorAll('tbody [type="checkbox"]');
      if (Array.from(trList).every((item: HTMLInputElement) => item.checked)) {
  ...
  _checkAllTable(selectedBar, tableCheckboxes, headerCheckbox, elemName) {
        ...
        otherElems.forEach((element) => {
          if (element !== el) {
            if (
              ...
              mathRound(
                element[scroll + Left] -
                (scrollX = element.eX = mathRound(xRate * (element[scroll + Width] - element[client + Width]))),
              )
            ) {
            ...
            if (
              ...
              mathRound(
                element[scroll + Top] -
                (scrollY = element.eY = mathRound(yRate * (element[scroll + Height] - element[client + Height]))),
              )
            ) {
