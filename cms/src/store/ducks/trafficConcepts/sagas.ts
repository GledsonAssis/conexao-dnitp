...
export function* fetchOne({ payload }: any) {
  if (payload) {
    ...
    try {
      const response: any = yield* call(service.client.get, `/trafficConcepts/${id}`);
      ...
      if (!err.error?.response?.data?.message || !err.error?.response?.data?.code) {
        parser = yield parseError(err.response, types.TrafficConcepts);
      }
...
export function* deleteOne({ payload }: any) {
  if (payload) {
    ...
    try {
      const response: any = yield* call(service.client.delete, `/cms/trafficConcepts/${id}`);
...
export function* submit({ payload }: any) {
  try {
    ...
    const response: any = yield* call(service.client[methodType], `/cms/trafficConcepts`, params);
